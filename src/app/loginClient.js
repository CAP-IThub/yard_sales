...
export default function LoginClient() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(false);
    if (result?.ok) setStatus('If your email is valid and active, a magic link has been sent.');
    else setStatus('There was a problem sending the magic link. Please try again.');
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <form onSubmit={handleSubmit} className="bg-neutral-950/80 backdrop-blur-xl p-8 rounded-xl border border-neutral-800 shadow-2xl w-full max-w-md space-y-6 text-[var(--foreground)] relative overflow-hidden">
        ...
        <input type="email" required placeholder="you@capplc.com" value={email} onChange={e=>setEmail(e.target.value)} className="w-full bg-neutral-800/70 border border-neutral-600 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-400 placeholder-neutral-400" />
        <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-500 transition font-medium">{loading ? 'Sending...' : 'Send Magic Link'}</button>
